...
export const getRevueAPI = async (path: string) => {
  const result = await fetch(`https://www.getrevue.co/api/v2/${path}`, {
    method: "POST",
    headers: {
...
export const getRevueLatest = async () => {
  return getRevueAPI("issues/latest");
};
